...
SELECT 
  ...
  CONCAT('Liquidación ', COALESCE(l.numero_liquidacion, CONCAT('LIQ-', l.id)), ' - Categoría ', COALESCE(cp.nombre, 'Sin categoría'), ' - Lote ', lt.numero_lote) AS concepto,
  l.observaciones,
...
SELECT 
  ...
  per.nombre_completo AS persona_nombre,
  'productor' AS persona_tipo,
  CONCAT('Pesaje categoría ', COALESCE(cp.nombre, 'Sin categoría'), ' - Lote ', l.numero_lote) AS concepto,
  'migracion' AS usuario_registro
